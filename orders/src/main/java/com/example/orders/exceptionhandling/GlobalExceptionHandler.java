...
public class GlobalExceptionHandler {
    ...
    private ResponseEntity<Object> buildResponse(Exception ex, HttpStatus defaultHttpStatus) {
        OrdersApiException response = buildErrorResponse(ex, defaultHttpStatus);
        return ResponseEntity.status(response.getStatus()).body(response);
    }

    private OrdersApiException buildErrorResponse(Exception ex, HttpStatus defaultHttpStatus) {
        ...
        return OrdersApiException.builder()
                .status(status)
                .error(error)
                .message(message)
