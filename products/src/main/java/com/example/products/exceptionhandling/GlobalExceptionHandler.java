...
public class GlobalExceptionHandler {
    ...
    private ResponseEntity<Object> buildResponse(Exception ex, HttpStatus defaultHttpStatus) {
        ProductsApiException response = buildErrorResponse(ex, defaultHttpStatus);
        return ResponseEntity.status(response.getStatus()).body(response);
    }

    private ProductsApiException buildErrorResponse(Exception ex, HttpStatus defaultHttpStatus) {
        ...
        return ProductsApiException.builder()
                .status(status)
                .error(error)
                .message(message)
