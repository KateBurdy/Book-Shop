...
public class GlobalExceptionHandler {
    ...
    private ResponseEntity<Object> buildResponse(Exception ex, HttpStatus defaultHttpStatus) {
        FilesApiException response = buildErrorResponse(ex, defaultHttpStatus);
        return ResponseEntity.status(response.getStatus()).body(response);
    }

    private FilesApiException buildErrorResponse(Exception ex, HttpStatus defaultHttpStatus) {
        ...
        return FilesApiException.builder()
                .status(status)
                .error(error)
                .message(message)
