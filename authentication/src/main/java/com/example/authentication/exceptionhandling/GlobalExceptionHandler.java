...
public class GlobalExceptionHandler {
    ...
    private ResponseEntity<Object> buildResponse(Exception ex, HttpStatus defaultHttpStatus) {
        AuthApiException response = buildErrorResponse(ex, defaultHttpStatus);
        return ResponseEntity.status(response.getStatus()).body(response);
    }

    private AuthApiException buildErrorResponse(Exception ex, HttpStatus defaultHttpStatus) {
        ...
        return AuthApiException.builder()
                .status(status)
                .error(error)
                .message(message)
