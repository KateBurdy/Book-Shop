...
public class GlobalExceptionHandler {
    ...
    private ResponseEntity<Object> buildResponse(Exception ex, HttpStatus defaultHttpStatus) {
        AuthorsApiException response = buildErrorResponse(ex, defaultHttpStatus);
        return ResponseEntity.status(response.getStatus()).body(response);
    }

    private AuthorsApiException buildErrorResponse(Exception ex, HttpStatus defaultHttpStatus) {
        ...
        return AuthorsApiException.builder()
                .status(status)
                .error(error)
                .message(message)
