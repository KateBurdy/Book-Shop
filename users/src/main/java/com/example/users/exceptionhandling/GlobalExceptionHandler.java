...
public class GlobalExceptionHandler {
    ...
    private ResponseEntity<Object> buildResponse(Exception ex, HttpStatus defaultHttpStatus) {
        UsersApiException response = buildErrorResponse(ex, defaultHttpStatus);
        return ResponseEntity.status(response.getStatus()).body(response);
    }

    private UsersApiException buildErrorResponse(Exception ex, HttpStatus defaultHttpStatus) {
        ...
        return UsersApiException.builder()
                .status(status)
                .error(error)
                .message(message)
